...
class SensorFusionPipeline:
    ...
    def process_imu(self, gyro: np.ndarray, accel: np.ndarray, mag: np.ndarray,
                    prefilter: str = "lpf") -> dict:
        """
        Call this at IMU rate.
        Returns current fused state + recommended SRT 204 fields.
        """
        ...
        return {"state": state, "srt204": srt204, "heading_rad": heading_rad}
...
# ------------------------------------------------------------------
# Self-contained demo (uses synthetic IMU + occasional GPS seeds)
# ------------------------------------------------------------------
if __name__ == "__main__":
    pipe = SensorFusionPipeline(imu_dt=0.01)

    # Simulate 2 seconds @ 100 Hz + GPS every 0.5 s
    print("Running fusion pipeline demo (synthetic)...\n")
    for i in range(200):
        ...
        res = pipe.process_imu(gyro, accel, mag, prefilter="lpf")
